...
static inline long sc_klog(const char *key, const char *msg)
{
    ...
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_KLOG), msg);
    return ret;
}
...
{
    if (!key || !key[0]) return -EINVAL;
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_KERNELPATCH_VER));
    return (uint32_t)ret;
}
...
{
    if (!key || !key[0]) return -EINVAL;
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_KERNEL_VER));
    return (uint32_t)ret;
}
...
static inline long sc_su(const char *key, struct su_profile *profile)
{
    ...
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_SU), profile);
    return ret;
}
...
{
    if (!key || !key[0]) return -EINVAL;
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_SU_TASK), tid, profile);
    return ret;
}
...
static inline long sc_kpm_load(const char *key, const char *path, const char *args, void *reserved)
{
    ...
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_KPM_LOAD), path, args, reserved);
    return ret;
}
...
static inline long sc_kpm_control(const char *key, const char *name, const char *ctl_args, char *out_msg, long outlen)
{
    ...
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_KPM_CONTROL), name, ctl_args, out_msg, outlen);
    return ret;
}
...
static inline long sc_kpm_unload(const char *key, const char *name, void *reserved)
{
    if (!key || !key[0]) return -EINVAL;
    if (!name || strlen(name) <= 0) return -EINVAL;
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_KPM_UNLOAD), name, reserved);
    return ret;
}
...
{
    if (!key || !key[0]) return -EINVAL;
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_KPM_NUMS));
    return ret;
}
...
static inline long sc_kpm_list(const char *key, char *names_buf, int buf_len)
{
    ...
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_KPM_LIST), names_buf, buf_len);
    return ret;
}
...
static inline long sc_kpm_info(const char *key, const char *name, char *buf, int buf_len)
{
    ...
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_KPM_INFO), name, buf, buf_len);
    return ret;
}
...
{
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_PANIC));
    return ret;
}

static inline long __sc_test(const char *key, long a1, long a2, long a3)
{
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_TEST), a1, a2, a3);
    return ret;
}
...
{
    if (!key || !key[0]) return -EINVAL;
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_SU_GRANT_UID), uid, profile);
    return ret;
}
...
{
    if (!key || !key[0]) return -EINVAL;
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_SU_REVOKE_UID), uid);
    return ret;
}
...
{
    if (!key || !key[0]) return -EINVAL;
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_SU_NUMS));
    return ret;
}
...
static inline long sc_su_allow_uids(const char *key, uid_t *buf, int num)
{
    ...
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_SU_LIST), buf, num);
    return ret;
}
...
{
    if (!key || !key[0]) return -EINVAL;
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_SU_PROFILE), uid, out_profile);
    return ret;
}
...
static inline long sc_su_reset_path(const char *key, const char *path)
{
    ...
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_SU_RESET_PATH), path);
    return ret;
}
...
static inline long sc_su_get_path(const char *key, char *buf, int buf_size)
{
    ...
    long ret = syscall(__NR_supercall, key, hash_key_cmd(key, SUPERCALL_SU_GET_PATH), buf, buf_size);
    return ret;
}
